...
class PermissionExpired extends Notification
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
                    ->subject("Permiso de Unidad Expirada - " . getenv("APP_NAME"))
                    ->line(new HtmlString("- <strong>Unidad:</strong> {$this->data['vehicle_label']}"))
                    ->line(new HtmlString("- <strong>Concepto:</strong> {$this->data['concept']}"))
    ...
    {
        $body = "Concepto: {$this->data['concept']}. Fecha: {$this->data['expired_at']}. Unidad: {$this->data['vehicle_label']}. Chofer: {$this->data['name']}}";

        return OneSignalMessage::create()
            ->setSubject("Permiso de Unidad Expirada - Mayocabo")
            ->setBody($body)
