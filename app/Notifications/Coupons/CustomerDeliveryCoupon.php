...
class CustomerDeliveryCoupon extends Notification
{
    ...
    public function __construct($data)
    {
        ...
        if(!array_key_exists("profile_link", $data)){
            $this->data["profile_link"] = true;
        }

        if(!array_key_exists("attach", $data)){
            $this->data["attach"] = false;
        }
    ...
    public function toMail($notifiable)
    {
        ...
        if($this->data["attach"]){
            $message->attachData($this->data["attach"], "entrega-{$this->data['folio']}.pdf");
        }
        
        if($this->data["profile_link"]){
            $message->line(new HtmlString("Recuerde que puede revisar su balance mediante el siguiente enlace:"))
